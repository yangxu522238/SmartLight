...
#if defined(DEBUG_ENABLE) && defined(ENABLE_TEMPERATURE_CONTROLLER) && !defined(CSR101x_A05)
 ...
 *      Nothing
 *----------------------------------------------------------------------------*/
static void appUartProcessEvent(msg_t *msg)
{
    /* UART message type */
    uart_msg_t *uart_msg;
    ...
    switch(msg->header.id)
    {
        ...
        case UART_READ_IND:
            UartDataRxCallback(uart_msg->body.read_ind.buffer, 
