...
/* Device Apprearance. */
CSR_MESH_DEVICE_APPEARANCE_T appearance = {"Beacon2.1",
                                          {APPEARANCE_ORG_BLUETOOTH_SIG,
                                           APPEARANCE_CSRMESH_BEACON_VALUE,
                                           0x00000000}};

CSR_SCHED_LE_PARAMS_T                    le_params;

#ifdef ENABLE_BEACON_MODEL
BEACON_HANDLER_DATA_T                    g_beacon_handler_data;
...
extern void AppMeshInit(void)
{
    ...
    if(result == CSR_MESH_RESULT_SUCCESS)
    {
        ...
        /* Do not start CSRmesh. Depending on the association status the beacon
         * model handler will either start beacons or mesh. If the device is in
         * unassociated state the beacon handler would initiate association.
         */
        if(g_mesh_handler_data.assoc_state == app_state_associated)
        {
            DEBUG_STR("Beacon is associated\r\n");

            /* Initialize the source sequence cache */
            AppInitializeSeqCache(FALSE);

            /* Set the beacon device mode to mesh */
            BeaconSetMode(device_mode_mesh);
        }
        else if(g_app_nvm_fresh == TRUE)
        {
