...
extern void IOTRelayInit(void)
{
    ...
    pio_msk_rel.mask[0] = 0;
    pio_msk_rel.mask[1] = 0;
    pio_msk_rel.mask[2] = 0;

    /* Set the mask for the button PIO */
    sw4_mask_idx = setPioMask(REL_PIO, &pio_msk_rel);

    /* Set the dirs mask to all 1 (outputs) */
    dirs.mask[0] = 1;
    dirs.mask[1] = 1;
    dirs.mask[2] =1;

    /* Configure button PIO as an input */
    PioSetDirMultiple(pio_msk_rel, dirs);
    /* Configure pull-up on button PIO */
    PioSetPullModeMultiple(pio_msk_rel, pio_mode_strong_pull_up);
    /* Event mode selection */    
    PioSet(REL_PIO, 0);    
}

extern void IOTRelayInit2(void)
{
    PioSetDir(REL_PIO, PIO_DIRECTION_OUTPUT);
    RelayStatus=0x00;         
    /* Set the LED to OFF on Initialization */
    PioSet(REL_PIO, 0);
}
extern void IOTRelayOpen(void)
{
    RelayStatus=0x01;     
    PioSet(REL_PIO, 1);
}
extern void IOTRelayClose(void)
{
    RelayStatus=0x00;     
    PioSet(REL_PIO, 0);
}
