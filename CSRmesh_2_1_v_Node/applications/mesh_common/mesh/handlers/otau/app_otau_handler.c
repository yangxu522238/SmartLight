...
static void otauCallbackValidateHeaderBody(GAIA_OTAU_EVENT_VALIDATE_HEADER_BODY_T *event_data)
{
    ...
    while(size != 0)
    {
        switch (g_app_otau_data.data_type)
        {
            ...
            {
                uint8 device_variant = unknown_device;
#ifdef GAIA_OTAU_RELAY_SUPPORT
...
/*-----------------------------------------------------------------------------*
 *  NAME
 *      GAIAInPostRebootPhase
 *
 *  DESCRIPTION
 *      This function returns TRUE if GAIA service is in POST REBOOT phase
 *
 *  RETURNS/MODIFIES
 *      Boolean
 *
 *----------------------------------------------------------------------------*/
extern bool GAIAInPostRebootPhase(void)
{
    uint16 resume_point = 0;
    
    /* Read the resume point from NVM */
    Nvm_Read((uint16*)&resume_point,
                 sizeof(resume_point),
                 NVM_OFFSET_GAIA_OTA_SERVICE + GAIA_NVM_RESUME_POINT_OFFSET);
    
    /* Check if GAIA is in POST REBOOT phase. If GAIA service is in Post reboot
     * phase, reinitialise the nvm sanity and app nvm version in NVM so as
     * to prevent incorrect reading from nvm offsets.
     */
    if(resume_point == UPGRADE_RESUME_POINT_POST_REBOOT)
    {
        uint16 nvm_sanity = 0xffff,nvm_version = 0xffff ;
        Nvm_Write(&nvm_sanity, sizeof(nvm_sanity), NVM_OFFSET_SANITY_WORD);
        Nvm_Write(&nvm_sanity, sizeof(nvm_sanity), CSR_MESH_NVM_SANITY_WORD_OFFSET);
        Nvm_Write(&nvm_version, sizeof(nvm_version), NVM_OFFSET_APP_NVM_VERSION);
        return TRUE;
    }
