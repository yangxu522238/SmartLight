...
#ifdef APP_BEACON_MODE
            p_beacon_hdlr_data->beacons[index].payloadlength 
                                       = sizeof(g_default_eddystone_url);
...
static CSRmeshResult beaconModelEventHandler(CSRMESH_MODEL_EVENT_T event_code, 
                                             ...
                                             void **state_data)
{
    CSR_MESH_THREAD_LOCAL(CSRMESH_BEACON_SET_PAYLOAD_T, p_params);
    switch(event_code)
    {
        ...
        case CSRMESH_BEACON_BEACON_STATUS:
        {
            ...
            {
               CSR_MESH_THREAD_LOCAL(uint16, groups[MAX_MANAGED_BEACON_GRPS+1]);
               uint16 grp_cnt=0, grp_idx=0;
