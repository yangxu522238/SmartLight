...
#define CSR_MESH_LUMICAST_PID   (0x1067)

/* Vendor ID for CSR */
#define APP_VENDOR_ID           (0x0A12)

/* Product ID. */
#define APP_PRODUCT_ID          (CSR_MESH_LUMICAST_PID)
...
/* Time in (10 Seconds) for which the device will stay in mesh mode */
#define DEFAULT_MESH_ON_TIME    (3) /* 30 Seconds */
 
/* Time interval (minutes) at which the mesh wakes-up in mesh mode  */
#define DEFAULT_MESH_INTERVAL   (5) /* 5 Minutes */

/* Beacon types */
#define BEACON_TYPE_SUPPORTED   (1 << csr_mesh_beacon_type_lumicast)

#define MAX_BEACONS_SUPPORTED   (1)

#define APP_LUMICAST_MODE

/* Make sure that the payload size is always even as this value is used for
 * packing and unpacking of payload while writing and reading onto NVM. The Max
 * payload size currently supported by library is 30.
 */
